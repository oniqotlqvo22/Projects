...
struct Operations {
    ...
    enum Movies {
        ...
        func chooseModel() -> ModelType.Type {
            ...
            case .fetchMovieCast:
                return MovieCast.self
            case .fetchMovieVideos:
                return MovieVideos.self
            case .searchMovieByTitle:
                return MoviesData.self
            }
        }
    ...
    enum TVSeries {
        ...
        func chooseModel() -> ModelType.Type {
            ...
            case .fetchSeriesCast:
                return TVSeriesCastData.self
            case .fetchSeriesVideos:
                return TVSeriesVideosData.self
            case .searchSeriesByTitle:
                return TVSeriesData.self
            }
        }
...
enum APIOperations {
    ...
    func chooseOperation(for mediaType: MediaType, key: String, page: Int) -> String {
        ...
        switch self {
        case .fetchSeries:
            return "\(baseURL)tv/\(key)?language=en-US&page=\(page)"
        case .fetchSeriesDetails:
            return "\(baseURL)tv/\(key)?language=en-US"
        case .fetchSeriesCast:
            return "\(baseURL)tv/\(key)/credits?language=en-US"
        case .fetchSeriesVideos:
            return "\(baseURL)tv/\(key)/videos?language=en-US"
        case .searchSeriesByTitle:
            return "\(baseURL+"search/")tv?query=\(key)&include_adult=false&language=en-US&page=\(page)"
        case .fetchMovies:
            return "\(baseURL)movie/\(key)?language=en-US&page=\(page)"
        case .fetchMovieDetails:
            return "\(baseURL)movie/\(key)?language=en-US"
        case .fetchMovieCast:
            return "\(baseURL)movie/\(key)/credits?language=en-US"
        case .fetchMovieVideos:
            return "\(baseURL)movie/\(key)/videos?language=en-US"
        case .searchMovieByTitle:
            return "\(baseURL+"search/")movie?query=\(key)&include_adult=false&language=en-US&page=\(page)"
        }
